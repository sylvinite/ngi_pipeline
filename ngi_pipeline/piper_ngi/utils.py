...
def create_log_file_path(project, sample=None, libprep=None, seqrun=None, workflow_name=None):
...
def create_exit_code_file_path(project, sample=None, libprep=None, seqrun=None, workflow_name=None):
    base_path = os.path.join(project.base_path, "ANALYSIS", project.name, "logs")
    file_name = project.name
    if sample:
        file_name += "-{}".format(sample.name)
